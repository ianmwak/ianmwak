...
- **TrueNAS** — Setting up a NAS with the new 4TB drive, evaluating dedicated low-power hardware vs VM
- **Meshnet** — NordVPN meshnet on the server for remote status monitoring
- **3D Printer Repair** — Picked up a broken printer for $15, documenting the fix
...
`Proxmox` `Linux CLI` `Pi-hole` `DNS` `Networking` `Python` `Raspberry Pi` `Home Infrastructure` `VM Deployment` `Container Setup`
